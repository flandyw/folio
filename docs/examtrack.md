...
Deleting a card in Folio (mid-review or from its details) removes it locally at once, drops any queued rating, keeps handwriting on this device, and queues a conditional remote `deleted_at` write for the next sync. A queued delete is never resurrected by the still-active remote row; a conflicting concurrent web edit keeps the delete queued for a later sync.
