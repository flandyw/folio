...
@Composable
fun BookmarkListContent(notes: List<Notebook>, onOpen: (String, Int) -> Unit, modifier: Modifier = Modifier, onUnbookmark: ((String, String) -> Unit)? = null) {
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        Text("${entries.size} bookmarks across your library · long-press one to remove it", Modifier.padding(horizontal = 24.dp, vertical = 12.dp),
            ...
            items(entries, key = { (note, page) -> "${note.id}/${page.value.id}" }) { (note, entry) ->
                ...
                Box {
                    ...
                    DropdownMenu(menu, { menu = false }, modifier = Modifier.guardUiTouches()) {
                        DropdownMenuItem({ Text("Open page") }, { menu = false; onOpen(note.id, entry.index) })
                        if (onUnbookmark != null) DropdownMenuItem(
                            { Text("Remove bookmark") },
                            { menu = false; onUnbookmark(note.id, entry.value.id) },
