...
- Releases (`.github/workflows/release.yml`): version `0.2.N` / code `N` = `git rev-list --count HEAD`; keep `main` append-only. Signed RSA-3072 (`folio` alias) via `ANDROID_KEYSTORE_BASE64/PASSWORD` secrets; local build needs `ANDROID_KEYSTORE_PATH/PASSWORD + VERSION_CODE/NAME`, else `assembleRelease` fails. Each release verifies with `apksigner` and ships `SHA256SUMS`. Debug and release keys differ — uninstall debug (export first) before installing a release.
